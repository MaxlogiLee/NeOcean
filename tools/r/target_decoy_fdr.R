...
prt_N=foreach(j=1:length(seq),.combine = c) %do%  system(paste("LC_ALL=C grep -B1 ",seq[j]," ",target_db,"|grep '^>'|wc -l",sep=""),intern=T)
...
decoy_N=foreach(j=1:length(seq),.combine = c) %do% system(paste("LC_ALL=C grep -B1 ",seq[j]," ",decoy_db,"|grep '^>'|wc -l",sep=""),intern=T)


target_decoy=data.frame(Sequence=seq,Target=as.numeric(prt_N),Decoy=as.numeric(decoy_N))
