...
refBed = read.table(refbed_file, sep="\t", header = FALSE, stringsAsFactors = FALSE)
colnames(refBed) = c("chr", "start1", "end1", "start2", "end2", "strand", "XLOC", "TCONS", "coding", "start3", "end3")
...
for(j in 1:length(gl)){
  ...
  for(m in 1:nrow(loc)){
    ...
    # 使用TCONS或XLOC来选择匹配的行
    bed = refBed[refBed$TCONS == loc$tcons[m], ]
    exon_start = as.numeric(unlist(strsplit(bed$start3, split = ",")))
    exon_end = as.numeric(unlist(strsplit(bed$end3, split = ",")))
