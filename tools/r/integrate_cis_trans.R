...
write.table(cis_res,paste("res/10_","cis","_质谱结果_",nrow(cis_res),".xls",sep=""),sep="\t",row.names = F,quote = F)
write.table(trans_res,paste("res/10_","trans","_质谱结果_",nrow(trans_res),".xls",sep=""),sep="\t",row.names = F,quote = F)
write.table(ms,paste("res/11_","没有grep到的","_质谱结果_",nrow(ms),"_peplist.xls",sep=""),sep="\t",row.names = F,quote = F)
